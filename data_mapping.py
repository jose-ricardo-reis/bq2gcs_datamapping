...
def main(id_projeto):
    ...
    nome_arquivo_zip_final = f"{data_atual}_{id_projeto}.zip"

    # Exporta as informações do dataset para um arquivo CSV
    datasets = exportar_informacoes_do_dataset(id_projeto)
    arquivo_csv_datasets = f'{data_atual}_{id_projeto}_datasets.csv'
    ...
    for dataset in datasets[1:]:
        ...
        arquivo_csv_tabelas = f'{data_atual}_{id_projeto}_{nome_dataset}_tabelas.csv'
    ...
    for dataset in datasets[1:]:
        ...
        arquivo_csv_quantidade_colunas = f'{data_atual}_{id_projeto}_{nome_dataset}.csv'
    ...
    arquivo_zip_tabelas_dataset = f'{data_atual}_{id_projeto}_tabelas-dataset.zip'
    arquivo_zip_quantidade_colunas = f'{data_atual}_{id_projeto}_qtdade-colunas.zip'
    arquivo_zip_data_mapping = f'{data_atual}_{id_projeto}.zip'

    # Compacta os arquivos das tabelas dos datasets em um arquivo ZIP
    compactar_arquivos([arquivo_csv_tabelas for _, arquivo_csv_tabelas in tabelas_do_dataset],
                       arquivo_zip_tabelas_dataset)
    ...
    compactar_arquivos([arquivo_csv_datasets, arquivo_zip_tabelas_dataset, arquivo_zip_quantidade_colunas],
                       arquivo_zip_data_mapping)

    # Deleta os arquivos CSV, os arquivos ZIP das tabelas dos datasets e o arquivo ZIP da quantidade de colunas
    deletar_arquivos([arquivo_csv_datasets] + arquivos_quantidade_colunas + [arquivo_zip_tabelas_dataset,
                                                                             arquivo_zip_quantidade_colunas] + [
                         nome_arquivo_tabela for _, nome_arquivo_tabela in tabelas_do_dataset])

    # Envia o arquivo ZIP final para o bucket GCS
    upload_para_bucket(bucket, arquivo_zip_data_mapping)

    # Apaga arquivo da VM
    comando_rm = f"rm -r {data_atual}_{id_projeto}.zip"
...
if __name__ == '__main__':
    ...
    ''' Lembre de alterar as variávis
        id_projeto e bucket'''

    # Define o ID do projeto
    id_projeto = 'eneva-datahub-prv-dsv'
    # Define o caminho do bucket
    bucket = 'staging-datahub-provider-dsv/Data_Mapping'
